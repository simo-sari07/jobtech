...
import {
  X, FileText, Upload, CheckCircle2, AlertCircle, ChevronRight,
} from 'lucide-react'
...
export default function ApplyModal({ isOpen, onClose, job, onSuccess }: ApplyModalProps) {
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[92vh] overflow-y-auto animate-fade-up">
        ...
        <div className="sticky top-0 bg-white border-b border-slate-100 px-6 py-4 flex items-start justify-between z-10">
          <div className="flex-1 min-w-0 pr-4">
            <h2 className="font-bold text-slate-900 text-base">Apply for this position</h2>
            <p className="text-sm text-slate-500 mt-0.5 truncate">{job.title}</p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 flex flex-col gap-5">
              ...
              <div className="flex flex-col gap-2">
                ...
                >
                  <input
                    ...
                    onChange={() => { setCvChoice('saved'); setNewCvFile(null) }}
                    className="accent-blue-600 w-4 h-4 shrink-0"
                  />
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${cvChoice === 'saved' ? 'bg-blue-100' : 'bg-slate-100'}`}>
                      <FileText size={15} className={cvChoice === 'saved' ? 'text-blue-600' : 'text-slate-400'} />
                    </div>
                    <div className="min-w-0">
                      <p className={`text-sm font-semibold ${cvChoice === 'saved' ? 'text-blue-900' : 'text-slate-800'}`}>
                        Use your saved CV
                      </p>
                      <p className="text-xs text-slate-400 truncate">{savedCvName}</p>
                    </div>
                  </div>
                  {cvChoice === 'saved' && (
                ...
                >
                  ...
                  />
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <div className={`w-8 h-8 rounded-lg flex items-center justify-center shrink-0 ${cvChoice === 'new' ? 'bg-blue-100' : 'bg-slate-100'}`}>
                      <Upload size={15} className={cvChoice === 'new' ? 'text-blue-600' : 'text-slate-400'} />
                    </div>
                    <p className={`text-sm font-semibold ${cvChoice === 'new' ? 'text-blue-900' : 'text-slate-800'}`}>
                      Upload a new CV
                    </p>
                  </div>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                  ...
                  <p className="text-xs text-amber-700">
                    You don't have a saved CV yet. Please upload your CV to apply.
                  </p>
                </div>
                <FileDropZone
                  file={newCvFile}
          ...
          <div>
            <label className="text-xs font-semibold text-slate-500 uppercase tracking-wider block mb-2">
              Cover Letter <span className="text-slate-400 font-normal normal-case ml-1">(optional)</span>
            </label>
            <textarea
              value={coverLetter}
...
function FileDropZone({ file, dragOver, fileRef, onDragOver, onDragLeave, onDrop, onClick, onChange }: DropZoneProps) {
  return (
    <div
    ...
    >
      ...
      <input ref={fileRef} type="file" accept=".pdf,application/pdf" className="hidden" onChange={onChange} />
    </div>
  )
}
